...
"""for id in parcel_ids: #This is just a test for seeing how the parcel ids are being stores
    print(id) #Confirmed it stores in a list type of deal """
...
import matplotlib.pyplot as plt #Going to import again just in case we remove the previous import
import numpy as np #Using this to log all of the values to show a more representative scatter plot
# ---- Creating Scatter Plots ----

sample_clean = df_clean.sample(25000) #Sample the clean data frame just to show relationship
alpha_value = 0.3 #Alpha value will allow for the graph to look more readable, by making dense regions darker
#Scatter plot for Sqaure Footage vs Home Value 
plt.figure(figsize=(8, 6))
x = sample_clean['calculatedfinishedsquarefeet'].clip(upper=sample_clean['calculatedfinishedsquarefeet'].quantile(0.99)) #The property trait, cap the values at upper 99th percentile to show the relationship
y = sample_clean['taxvaluedollarcnt'].clip(upper=sample_clean['taxvaluedollarcnt'].quantile(0.99)) #The prooperty tax value, cap the values at upper 99th percentile to show the relationship
plt.scatter(np.log(x+1), np.log(y+1), alpha=alpha_value)
plt.xlabel("Finished Square Feet")
plt.ylabel("Property Tax Value ($)")
plt.title("Sqaure Footage vs. Tax Value")
plt.show()

#Scatter plot for Lot Size vs. Home Value 
plt.figure(figsize=(8, 6))
x = sample_clean['lotsizesquarefeet'].clip(upper=sample_clean['lotsizesquarefeet'].quantile(0.99)) #The property trait, cap the values at upper 99th percentile to show the relationship
y = sample_clean['taxvaluedollarcnt'].clip(upper=sample_clean['taxvaluedollarcnt'].quantile(0.99)) #The prooperty tax value, cap the values at upper 99th percentile to show the relationship
plt.scatter(np.log(x+1), np.log(y+1), alpha=alpha_value)
plt.xlabel("Lot Size")
plt.ylabel("Property Tax Value ($)")
plt.title("Lot Size vs. Tax Value")
plt.show()

#Scatter plot for Bedrooms vs Home Value 
plt.figure(figsize=(8, 6))
x = sample_clean['bedroomcnt'].clip(upper=sample_clean['bedroomcnt'].quantile(0.99)) #The property trait, cap the values at upper 99th percentile to show the relationship
y = sample_clean['taxvaluedollarcnt'].clip(upper=sample_clean['taxvaluedollarcnt'].quantile(0.99)) #The prooperty tax value, cap the values at upper 99th percentile to show the relationship
plt.scatter(np.log(x+1), np.log(y+1), alpha=alpha_value)
...
#Scatter plot for Bathrooms vs. Home Value 
plt.figure(figsize=(8, 6))
x = sample_clean['bathroomcnt'].clip(upper=sample_clean['bathroomcnt'].quantile(0.99)) #The property trait, cap the values at upper 99th percentile to show the relationship
y = sample_clean['taxvaluedollarcnt'].clip(upper=sample_clean['taxvaluedollarcnt'].quantile(0.99)) #The prooperty tax value, cap the values at upper 99th percentile to show the relationship
plt.scatter(np.log(x+1), np.log(y+1), alpha=alpha_value)
...
#Scatter plot for Year Built vs. Home Value 
plt.figure(figsize=(8, 6))
x = sample_clean['yearbuilt'] #Dont clip the year because all years are relvant and not extreme 
y = sample_clean['taxvaluedollarcnt'].clip(upper=sample_clean['taxvaluedollarcnt'].quantile(0.99)) #The prooperty tax value, cap the values at upper 99th percentile to show the relationship
plt.scatter(np.log(x+1), np.log(y+1), alpha=alpha_value)
...
# --- One Hot Encode Property Land USe Type ID ---
encoded_df = pd.get_dummies(df_clean, columns=['propertylandusetypeid'], prefix='plti') #Creates numeric represnetaions of the category the plti fits into, type1, type2, etc.
print(f"One-Hot Encoded Sample of Property Land User Type ID:\n {encoded_df.head()}")
...
#TL;DR - Georgia Week 1 Tasks
"""
1. Test for how parcel id's are stored on line 42: Confirmed that they are stored should we need to access them later
2. Line 175-229: Created various scatter plots comparing charcatertics to tax dolar values
3. Line 231-: One Hot Encoded the propertylandusetypeid
"""